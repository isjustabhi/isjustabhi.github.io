...
function TimelineItem({ item, index }) {
  ...
  return (
    <div ref={ref} className="relative flex items-start md:items-center gap-4 md:gap-0 mb-12 last:mb-0">
      {/* Desktop layout */}
      <div className="hidden md:grid md:grid-cols-[1fr_60px_1fr] w-full items-center">
        ...
        <div className="flex justify-center">
          <motion.div
            ...
            className="w-10 h-10 rounded-full glass border border-neon-cyan/30 flex items-center justify-center z-10 neon-glow"
          >
      ...
      <div className="md:hidden flex gap-4 w-full">
        <div className="flex flex-col items-center">
          <motion.div
            ...
            className="w-8 h-8 rounded-full glass border border-neon-cyan/30 flex items-center justify-center z-10 shrink-0"
          >
...
function CardContent({ item, align }) {
  return (
    <div
      className={`glass rounded-xl p-5 gradient-border group hover:bg-white/[0.02] transition-colors duration-300 ${
    ...
    >
      ...
      <div className={`flex flex-wrap gap-2 mt-3 ${align === 'right' ? 'justify-end' : 'justify-start'}`}>
        {item.highlights.map(h => (
          <span key={h} className="text-[10px] px-2 py-0.5 rounded-full bg-neon-cyan/10 text-neon-cyan/80 border border-neon-cyan/10">
            {h}
          </span>
        ))}
      </div>
